...
{
    printf("Calculating the first ten digits of the sum of the pre-given one-hundred 50-digit numbers.\n");

    unsigned sum[100] = { 0u };
    unsigned carry = 0u;
    unsigned digitcount = sizeof(digits) / sizeof(digits[0]);
    unsigned digitlen = sizeof(digits[0]) / sizeof(digits[0][0]);
    unsigned sumoffset = 100 - digitlen; 
    ...
    for (int j = digitlen - 1; j >= 0; j--)
    {
        ...
        digit res = sum[j + sumoffset] + digits[i][j];
        sum[j + sumoffset] = res % 10u;
        digit carry = res / 10u;

        unsigned reps = 0u;
        for (int k = j + sumoffset - 1; k >= 0; k--)
        {
            if (carry == 0) break;
            digit res = sum[k] + carry;
            carry = res / 10u;
            sum[k] = res % 10u;
            reps++;
        }
        unsigned maxidx = digitlen - j + reps;
        if (sumlen < maxidx) sumlen = maxidx;
    }

    printf("Sum: ");
    for (unsigned i = 100 - sumlen; i < 100; i++) printf("%d", sum[i]);
    printf("\nResult: ");
    for (unsigned i = 0; i <= 10; i++) printf("%d", sum[100 - sumlen + i]);
    printf("\n");
}
